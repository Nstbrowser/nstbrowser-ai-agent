...
impl DaemonState {
    pub fn new() -> Self {
        Self {
            ...
            ref_map: RefMap::new(),
            domain_filter: env::var("NSTBROWSER_AI_AGENT_ALLOWED_DOMAINS")
                .ok()
                .filter(|s| !s.is_empty())
                .map(|s| DomainFilter::new(&s)),
            event_tracker: EventTracker::new(),
            session_name: env::var("NSTBROWSER_AI_AGENT_SESSION_NAME").ok(),
            session_id: env::var("NSTBROWSER_AI_AGENT_SESSION")
...
async fn auto_launch(state: &mut DaemonState) -> Result<(), String> {
    ...
    if let Ok(cdp) = env::var("NSTBROWSER_AI_AGENT_CDP") {
    ...
    if env::var("NSTBROWSER_AI_AGENT_AUTO_CONNECT").is_ok() {
...
fn launch_options_from_env() -> LaunchOptions {
    let headed = env::var("NSTBROWSER_AI_AGENT_HEADED")
        .map(|v| v == "1" || v == "true")
        .unwrap_or(false);
    ...
    LaunchOptions {
        ...
        allow_file_access: env::var("NSTBROWSER_AI_AGENT_ALLOW_FILE_ACCESS")
            .map(|v| v == "1" || v == "true")
            .unwrap_or(false),
        args: env::var("NSTBROWSER_AI_AGENT_ARGS")
            .map(|v| {
                v.split([',', '\n'])
            ...
            .unwrap_or_default(),
        extensions: env::var("NSTBROWSER_AI_AGENT_EXTENSIONS").ok().map(|v| {
            v.split([',', '\n'])
                .map(|s| s.trim().to_string())
                .filter(|s| !s.is_empty())
                .collect()
        }),
        storage_state: env::var("NSTBROWSER_AI_AGENT_STATE").ok(),
        user_agent: env::var("NSTBROWSER_AI_AGENT_USER_AGENT").ok(),
        ignore_https_errors: env::var("NSTBROWSER_AI_AGENT_IGNORE_HTTPS_ERRORS")
            .map(|v| v == "1" || v == "true")
            .unwrap_or(false),
        color_scheme: env::var("NSTBROWSER_AI_AGENT_COLOR_SCHEME").ok(),
        download_path: env::var("NSTBROWSER_AI_AGENT_DOWNLOAD_PATH").ok(),
    }
}

fn daemon_state_from_env(state: &mut DaemonState) {
    if let Ok(name) = env::var("NSTBROWSER_AI_AGENT_SESSION_NAME") {
        if !name.is_empty() {
            state.session_name = Some(name);
        }
    }
    if let Ok(domains) = env::var("NSTBROWSER_AI_AGENT_ALLOWED_DOMAINS") {
        if !domains.is_empty() {
...
async fn handle_launch(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
    ...
    let executable_path: Option<String> = cmd
        ...
        .and_then(|v| v.as_str())
        .map(String::from)
        .or_else(|| std::env::var("NSTBROWSER_AI_AGENT_EXECUTABLE_PATH").ok());
    ...
    let options = LaunchOptions {
        ...
        executable_path: cmd
            ...
            .and_then(|v| v.as_str())
            .map(|s| s.to_string())
            .or_else(|| env::var("NSTBROWSER_AI_AGENT_EXECUTABLE_PATH").ok()),
...
async fn handle_getbyrole(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
    ...
    let js = format!(
        r#"(() => {{
            ...
            for (const el of els) {{
                if ({name_match}) {{
                    el.setAttribute('data-nstbrowser-ai-agent-located', 'true');
                    return true;
                }}
            }}
            return false;
    ...
    if let Some(ref browser) = state.browser {
        if let Ok(sid) = browser.active_session_id() {
            let _ = browser
                .evaluate(
                    "document.querySelector('[data-nstbrowser-ai-agent-located]')?.removeAttribute('data-nstbrowser-ai-agent-located')",
                    None,
                )
                .await;
            let _ = sid;
    ...
    let query = match strategy {
        "label" => format!(
            r#"(() => {{
                ...
                if (target) {{ target.setAttribute('data-nstbrowser-ai-agent-located', 'true'); return true; }}
                return false;
            }})()"#,
            match_fn = match_fn,
        ),
        "placeholder" => format!(
            r#"(() => {{
                ...
                if (el) {{ el.setAttribute('data-nstbrowser-ai-agent-located', 'true'); return true; }}
                return false;
            }})()"#,
...
async fn handle_nth(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
    ...
    let js = format!(
        r#"(() => {{
            ...
            els[idx].setAttribute('data-nstbrowser-ai-agent-located', 'true');
            return true;
        }})()"#,
        sel = serde_json::to_string(selector).unwrap_or_default(),
        idx = index,
    ...
    let located = "[data-nstbrowser-ai-agent-located='true']";
