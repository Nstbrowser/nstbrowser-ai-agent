...
pub async fn run_daemon(session: &str) {
    ...
    if let Ok(days_str) = env::var("NSTBROWSER_AI_AGENT_STATE_EXPIRE_DAYS") {
...
fn get_daemon_socket_dir() -> PathBuf {
    ...
    if let Ok(xdg) = env::var("XDG_RUNTIME_DIR") {
        if !xdg.is_empty() {
            return PathBuf::from(xdg).join("nstbrowser-ai-agent");
        }
    }
    ...
    std::env::temp_dir().join("nstbrowser-ai-agent")
}
