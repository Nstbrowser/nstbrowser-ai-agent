...
pub async fn save_state(
    ...
    if let Ok(key) = std::env::var("NSTBROWSER_AI_AGENT_ENCRYPTION_KEY") {
...
pub async fn load_state(client: &CdpClient, session_id: &str, path: &str) -> Result<(), String> {
    let json_str = if path.ends_with(".enc") {
        let key = std::env::var("NSTBROWSER_AI_AGENT_ENCRYPTION_KEY").map_err(|_| {
            "Encrypted state file requires NSTBROWSER_AI_AGENT_ENCRYPTION_KEY".to_string()
        })?;
        let data =
...
pub fn get_sessions_dir() -> PathBuf {
    if let Some(home) = dirs::home_dir() {
        home.join(".nstbrowser-ai-agent").join("sessions")
    } else {
        std::env::temp_dir()
...
mod tests {
    ...
    fn test_state_show_nonexistent_file() {
        let result = state_show("/tmp/nonexistent-nstbrowser-ai-agent-state-file.json");
        assert!(result.is_err());
    }

    #[test]
    fn test_state_clear_nonexistent_file() {
        let result = state_clear(Some("/tmp/nonexistent-nstbrowser-ai-agent-state-file.json"));
        assert!(result.is_err());
    }
    ...
    fn test_state_rename_nonexistent() {
        let result = state_rename(
            "/tmp/nonexistent-nstbrowser-ai-agent-state-file.json",
            "new-name",
        );
        assert!(result.is_err());
