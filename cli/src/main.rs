...
use flags::{clean_args, parse_flags, validate_nst_config};
    ...
    let script_path = match script_paths.iter().find(|p| p.exists()) {
        ...
        None => {
            if json_mode {
                ...
                eprintln!(
                    "{} auth-cli.js not found. Set NSTBROWSER_AI_AGENT_HOME or run from project directory.",
...
fn main() {
    ...
    if daemon_result.already_running {
        ...
        if !ignored_flags.is_empty() && !flags.json {
            eprintln!(
                "{} {} ignored: daemon already running. Use 'nstbrowser-ai-agent close' first to restart with new options.",
                color::warning_indicator(),
                ignored_flags.join(", ")
            );
        }
    ...
    let is_nst_api_command = cmd
        .get("action")
        .and_then(|v| v.as_str())
        .map(|action| action.starts_with("nst_"))
        .unwrap_or(false);
        ...
        if !is_nst_api_command {
            ...
            let err = match send_command(launch_cmd, &flags.session) {
                ...
                Ok(resp) => {
                    if env::var("NSTBROWSER_AI_AGENT_DEBUG").unwrap_or_default() == "1" {
                        eprintln!(
                            "[CLI DEBUG] Launch response success: {}, error: {:?}",
    ...
    match send_command(cmd.clone(), &flags.session) {
        Ok(resp) => {
            if env::var("NSTBROWSER_AI_AGENT_DEBUG").unwrap_or_default() == "1" {
                eprintln!("[CLI DEBUG] Command sent: {}", cmd);
                eprintln!(
                    "[CLI DEBUG] Response received: success={}, error={:?}",
                    resp.success, resp.error
                );
            }
            ...
            if flags.confirm_interactive {
                if let Some(data) = &resp.data {
                        ...
                        .unwrap_or(false)
                    {
                        ...
                        eprintln!("[nstbrowser-ai-agent] Action requires confirmation:");
