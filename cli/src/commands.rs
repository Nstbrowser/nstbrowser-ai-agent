    ...
    pub fn format(&self) -> String {
        match self {
            ...
            ParseError::MissingArguments { context, usage } => {
                format!(
                    "Missing arguments for: {}\nUsage: nstbrowser-ai-agent {}",
                    context, usage
                )
            }
            ParseError::InvalidValue { message, usage } => {
                format!("{}\nUsage: nstbrowser-ai-agent {}", message, usage)
            }
    ...
    match cmd {
            ...
            match sub {
                Some("save") => {
                    let name = rest.get(1).ok_or_else(|| ParseError::MissingArguments {
                        ...
                        usage: "nstbrowser-ai-agent auth save <name> --url <url> --username <user> --password <pass>",
                    })?;
                    ...
                    let url_val = url.ok_or_else(|| ParseError::MissingArguments {
                        context: "auth save".to_string(),
                        usage: "nstbrowser-ai-agent auth save <name> --url <url> --username <user> --password <pass> [--password-stdin]",
                    })?;
                ...
                Some("login") => {
                    let name = rest.get(1).ok_or_else(|| ParseError::MissingArguments {
                        ...
                        usage: "nstbrowser-ai-agent auth login <name>",
                    })?;
                ...
                Some("delete") | Some("remove") => {
                    let name = rest.get(1).ok_or_else(|| ParseError::MissingArguments {
                        ...
                        usage: "nstbrowser-ai-agent auth delete <name>",
                    })?;
                ...
                Some("show") => {
                    let name = rest.get(1).ok_or_else(|| ParseError::MissingArguments {
                        ...
                        usage: "nstbrowser-ai-agent auth show <name>",
                    })?;
        ...
        "confirm" => {
            let cid = rest.first().ok_or_else(|| ParseError::MissingArguments {
                ...
                usage: "nstbrowser-ai-agent confirm <confirmation-id>",
            })?;
        ...
        "deny" => {
            let cid = rest.first().ok_or_else(|| ParseError::MissingArguments {
                ...
                usage: "nstbrowser-ai-agent deny <confirmation-id>",
            })?;
...
fn parse_nst(rest: &[&str], id: &str) -> Result<Value, ParseError> {
    const VALID: &[&str] = &["browser", "profile"];

    match rest.first().copied() {
        Some("browser") => parse_nst_browser(&rest[1..], id),
        Some("profile") => parse_nst_profile(&rest[1..], id),
        Some(sub) => Err(ParseError::UnknownSubcommand {
            subcommand: sub.to_string(),
            valid_options: VALID,
        }),
        None => Err(ParseError::MissingArguments {
            context: "nst".to_string(),
            usage: "nst <browser|profile> <subcommand> [args...]",
        }),
    }
}
    ...
    match rest.first().copied() {
        ...
        Some("start") => {
            ...
            while i < rest.len() {
                match rest[i] {
                    "--headless" => {
                        options["headless"] = json!(true);
                    }
                    "--auto-close" => {
                        options["autoClose"] = json!(true);
                    }
                    "--disable-gpu" => {
                        options["disableGpu"] = json!(true);
                    }
                    "--proxy-enabled" => {
                        options["proxyEnabled"] = json!(true);
                    }
                    other => {
                        if other.starts_with("--") {
                            return Err(ParseError::InvalidValue {
...
fn parse_nst_profile(rest: &[&str], id: &str) -> Result<Value, ParseError> {
    ...
    match rest.first().copied() {
        ...
        Some("create") => {
            let name = rest.get(1).ok_or_else(|| ParseError::MissingArguments {
                context: "nst profile create".to_string(),
                usage: "nst profile create <name> [--platform <platform>] [--kernel <kernel>] [--group-id <id>] [--proxy-*]",
            })?;
            ...
            while i < rest.len() {
                match rest[i] {
                    "--proxy-host" => {
                        if let Some(host) = rest.get(i + 1) {
                            proxy_config["host"] = json!(host);
                            i += 1;
                        }
                    }
                    "--proxy-port" => {
                        if let Some(port) = rest.get(i + 1) {
                            if let Ok(port_num) = port.parse::<u16>() {
                                proxy_config["port"] = json!(port_num);
                                i += 1;
                            }
                        }
                    }
                    "--proxy-type" => {
                        if let Some(ptype) = rest.get(i + 1) {
                            proxy_config["type"] = json!(ptype);
                            i += 1;
                        }
                    }
                    "--proxy-user" | "--proxy-username" => {
                        if let Some(user) = rest.get(i + 1) {
                            proxy_config["username"] = json!(user);
                            i += 1;
                        }
                    }
                    "--proxy-password" => {
                        if let Some(pass) = rest.get(i + 1) {
                            proxy_config["password"] = json!(pass);
                            i += 1;
                        }
                    }
                    "--proxy-enabled" => {
                        proxy_config["enabled"] = json!(true);
                    }
                    "--platform" => {
                        if let Some(platform) = rest.get(i + 1) {
                    ...
                    "--kernel" => {
                        if let Some(kernel) = rest.get(i + 1) {
                            cmd["kernel"] = json!(kernel);
                            i += 1;
                        }
                    }
                    "--group-id" => {
                        if let Some(group_id) = rest.get(i + 1) {
                            cmd["groupId"] = json!(group_id);
                            i += 1;
                        }
                    }
                    other => {
                        if other.starts_with("--") {
                            return Err(ParseError::InvalidValue {
                                message: format!("unknown flag '{}' for nst profile create", other),
                                usage: "nst profile create <name> [--platform <platform>] [--kernel <kernel>] [--group-id <id>] [--proxy-*]",
                            });
                        }
                    }
            ...
            if !proxy_config.as_object().unwrap().is_empty() {
                cmd["proxyConfig"] = proxy_config;
            }

            Ok(cmd)
        }
        Some("delete") => {
            let profile_ids: Vec<&str> = rest[1..].iter().map(|s| s.as_ref()).collect();
        ...
        Some("proxy") => parse_nst_profile_proxy(&rest[1..], id),
        Some("tags") => parse_nst_profile_tags(&rest[1..], id),
        Some("groups") => parse_nst_profile_groups(&rest[1..], id),
        Some("cache") => {
            let sub = rest.get(1).copied();
            match sub {
                Some("clear") => {
                ...
                Some(s) => Err(ParseError::UnknownSubcommand {
...
fn parse_nst_profile_proxy(rest: &[&str], id: &str) -> Result<Value, ParseError> {
    const VALID: &[&str] = &["show", "update", "reset", "batch-update", "batch-reset"];
    ...
    match rest.first().copied() {
        ...
        None => Err(ParseError::MissingArguments {
            context: "nst profile proxy".to_string(),
            usage: "nst profile proxy <update|reset> [args...]",
        }),
    }
}
...
fn parse_nst_profile_groups(rest: &[&str], id: &str) -> Result<Value, ParseError> {
    const VALID: &[&str] = &["list", "change", "batch-change"];

    match rest.first().copied() {
        Some("list") | None => Ok(json!({ "id": id, "action": "nst_profile_groups_list" })),
        Some("batch-change") => {
...
mod tests {
    ...
    fn test_cookies_get_explicit() {
        let cmd = parse_command(&test_args("cookies get"), &default_flags()).unwrap();
        assert_eq!(cmd["action"], "cookies_get");
    }
    ...
    fn test_cookies_set_missing_value() {
        let result = parse_command(&test_args("cookies set mycookie"), &default_flags());
        assert!(result.is_err());
    }

    #[test]
    fn test_cookies_clear() {
        let cmd = parse_command(&test_args("cookies clear"), &default_flags()).unwrap();
    ...
    fn test_cookies_set_with_domain() {
        let cmd = parse_command(
            &test_args("cookies set mycookie myvalue --domain example.com"),
            &default_flags(),
        )
        .unwrap();
        assert_eq!(cmd["action"], "cookies_set");
    ...
    fn test_cookies_set_with_path() {
        let cmd = parse_command(
            &test_args("cookies set mycookie myvalue --path /api"),
            &default_flags(),
        )
        .unwrap();
        assert_eq!(cmd["action"], "cookies_set");
    ...
    fn test_cookies_set_with_httponly() {
        let cmd = parse_command(
            &test_args("cookies set mycookie myvalue --httpOnly"),
            &default_flags(),
        )
        .unwrap();
        assert_eq!(cmd["action"], "cookies_set");
    ...
    fn test_cookies_set_with_secure() {
        let cmd = parse_command(
            &test_args("cookies set mycookie myvalue --secure"),
            &default_flags(),
        )
        .unwrap();
        assert_eq!(cmd["action"], "cookies_set");
    ...
    fn test_cookies_set_with_samesite() {
        let cmd = parse_command(
            &test_args("cookies set mycookie myvalue --sameSite Strict"),
            &default_flags(),
        )
        .unwrap();
        assert_eq!(cmd["action"], "cookies_set");
    ...
    fn test_cookies_set_with_expires() {
        let cmd = parse_command(
            &test_args("cookies set mycookie myvalue --expires 1234567890"),
            &default_flags(),
        )
        .unwrap();
        assert_eq!(cmd["action"], "cookies_set");
    ...
    fn test_cookies_set_invalid_samesite() {
        let result = parse_command(
            &test_args("cookies set mycookie myvalue --sameSite Invalid"),
            &default_flags(),
        );
        assert!(result.is_err());
    }
    ...
    fn test_storage_local_get() {
        let cmd = parse_command(&test_args("storage local"), &default_flags()).unwrap();
    ...
    fn test_storage_local_get_key() {
        let cmd = parse_command(&test_args("storage local get mykey"), &default_flags()).unwrap();
        assert_eq!(cmd["action"], "storage_get");
        assert_eq!(cmd["type"], "local");
    ...
    fn test_storage_session_get() {
        let cmd = parse_command(&test_args("storage session"), &default_flags()).unwrap();
        assert_eq!(cmd["action"], "storage_get");
    ...
    fn test_storage_set_missing_value() {
        let result = parse_command(&test_args("storage local set mykey"), &default_flags());
        assert!(result.is_err());
    }

    #[test]
    fn test_storage_local_clear() {
        let cmd = parse_command(&test_args("storage local clear"), &default_flags()).unwrap();
    ...
    fn test_storage_session_clear() {
        let cmd = parse_command(&test_args("storage session clear"), &default_flags()).unwrap();
        assert_eq!(cmd["action"], "storage_clear");
        assert_eq!(cmd["type"], "session");
    }
    ...
    fn test_storage_invalid_type() {
        let result = parse_command(&test_args("storage invalid"), &default_flags());
        assert!(result.is_err());
    }
    ...
    #[test]
    fn test_back() {
        let cmd = parse_command(&test_args("back"), &default_flags()).unwrap();
        assert_eq!(cmd["action"], "back");
    }

    #[test]
    fn test_forward() {
        let cmd = parse_command(&test_args("forward"), &default_flags()).unwrap();
        assert_eq!(cmd["action"], "forward");
    }

    #[test]
    fn test_reload() {
        let cmd = parse_command(&test_args("reload"), &default_flags()).unwrap();
    ...
    #[test]
    fn test_click() {
        let cmd = parse_command(&test_args("click #button"), &default_flags()).unwrap();
    ...
    #[test]
    fn test_fill() {
        let cmd = parse_command(&test_args("fill #input hello world"), &default_flags()).unwrap();
    ...
    fn test_type_command() {
        let cmd = parse_command(&test_args("type #input some text"), &default_flags()).unwrap();
    ...
    #[test]
    fn test_select() {
        let cmd = parse_command(&test_args("select #menu option1"), &default_flags()).unwrap();
    ...
    #[test]
    fn test_frame_main() {
        let cmd = parse_command(&test_args("frame main"), &default_flags()).unwrap();
    ...
    #[test]
    fn test_tab_new() {
        let cmd = parse_command(&test_args("tab new"), &default_flags()).unwrap();
    ...
    #[test]
    fn test_tab_list() {
        let cmd = parse_command(&test_args("tab list"), &default_flags()).unwrap();
        assert_eq!(cmd["action"], "tab_list");
    }

    #[test]
    fn test_tab_switch() {
        let cmd = parse_command(&test_args("tab 2"), &default_flags()).unwrap();
    ...
    #[test]
    fn test_tab_close() {
        let cmd = parse_command(&test_args("tab close"), &default_flags()).unwrap();
    ...
    #[test]
    fn test_screenshot() {
        let cmd = parse_command(&test_args("screenshot"), &default_flags()).unwrap();
    ...
    fn test_screenshot_path() {
        let cmd = parse_command(&test_args("screenshot out.png"), &default_flags()).unwrap();
        assert_eq!(cmd["action"], "screenshot");
    ...
    fn test_screenshot_full_page() {
        let mut flags = default_flags();
        flags.full = true;
        let cmd = parse_command(&test_args("screenshot"), &flags).unwrap();
        assert_eq!(cmd["action"], "screenshot");
    ...
    fn test_screenshot_with_ref() {
        let cmd = parse_command(&test_args("screenshot @e1"), &default_flags()).unwrap();
        assert_eq!(cmd["action"], "screenshot");
    ...
    fn test_screenshot_with_css_class() {
        let cmd = parse_command(&test_args("screenshot .my-button"), &default_flags()).unwrap();
        assert_eq!(cmd["action"], "screenshot");
    ...
    fn test_screenshot_with_css_id() {
        let cmd = parse_command(&test_args("screenshot #header"), &default_flags()).unwrap();
        assert_eq!(cmd["action"], "screenshot");
    ...
    fn test_screenshot_with_path() {
        let cmd = parse_command(&test_args("screenshot ./output.png"), &default_flags()).unwrap();
        assert_eq!(cmd["action"], "screenshot");
        assert_eq!(cmd["selector"], serde_json::Value::Null);
    ...
    #[test]
    fn test_snapshot() {
        let cmd = parse_command(&test_args("snapshot"), &default_flags()).unwrap();
        assert_eq!(cmd["action"], "snapshot");
    }

    #[test]
    fn test_snapshot_interactive() {
        let cmd = parse_command(&test_args("snapshot -i"), &default_flags()).unwrap();
        assert_eq!(cmd["action"], "snapshot");
    ...
    fn test_snapshot_cursor() {
        let cmd = parse_command(&test_args("snapshot -C"), &default_flags()).unwrap();
        assert_eq!(cmd["action"], "snapshot");
    ...
    fn test_snapshot_interactive_cursor() {
        let cmd = parse_command(&test_args("snapshot -i -C"), &default_flags()).unwrap();
        assert_eq!(cmd["action"], "snapshot");
        assert_eq!(cmd["interactive"], true);
        assert_eq!(cmd["cursor"], true);
    }
    ...
    fn test_snapshot_compact() {
        let cmd = parse_command(&test_args("snapshot --compact"), &default_flags()).unwrap();
        assert_eq!(cmd["action"], "snapshot");
    ...
    fn test_snapshot_depth() {
        let cmd = parse_command(&test_args("snapshot -d 3"), &default_flags()).unwrap();
        assert_eq!(cmd["action"], "snapshot");
    ...
    fn test_wait_selector() {
        let cmd = parse_command(&test_args("wait #element"), &default_flags()).unwrap();
    ...
    fn test_wait_timeout() {
        let cmd = parse_command(&test_args("wait 5000"), &default_flags()).unwrap();
        assert_eq!(cmd["action"], "wait");
    ...
    #[test]
    fn test_wait_url() {
        let cmd = parse_command(&test_args("wait --url **/dashboard"), &default_flags()).unwrap();
    ...
    #[test]
    fn test_wait_load() {
        let cmd = parse_command(&test_args("wait --load networkidle"), &default_flags()).unwrap();
    ...
    fn test_wait_load_missing_state() {
        let result = parse_command(&test_args("wait --load"), &default_flags());
        assert!(result.is_err());
        assert!(matches!(
            result.unwrap_err(),
    ...
    #[test]
    fn test_wait_fn() {
        let cmd = parse_command(&test_args("wait --fn window.ready"), &default_flags()).unwrap();
    ...
    #[test]
    fn test_wait_text() {
        let cmd = parse_command(&test_args("wait --text Welcome"), &default_flags()).unwrap();
        assert_eq!(cmd["action"], "wait");
    ...
    fn test_record_start() {
        let cmd = parse_command(&test_args("record start output.webm"), &default_flags()).unwrap();
    ...
    fn test_record_start_with_url_no_protocol() {
        let cmd = parse_command(
            &test_args("record start demo.webm example.com"),
            &default_flags(),
        )
        .unwrap();
        assert_eq!(cmd["action"], "recording_start");
    ...
    fn test_record_start_missing_path() {
        let result = parse_command(&test_args("record start"), &default_flags());
        assert!(result.is_err());
        assert!(matches!(
            result.unwrap_err(),
            ParseError::MissingArguments { .. }
    ...
    #[test]
    fn test_record_stop() {
        let cmd = parse_command(&test_args("record stop"), &default_flags()).unwrap();
    ...
    fn test_record_restart_missing_path() {
        let result = parse_command(&test_args("record restart"), &default_flags());
        assert!(result.is_err());
        assert!(matches!(
            result.unwrap_err(),
            ParseError::MissingArguments { .. }
    ...
    fn test_record_invalid_subcommand() {
        let result = parse_command(&test_args("record foo"), &default_flags());
        assert!(result.is_err());
        assert!(matches!(
            result.unwrap_err(),
            ParseError::UnknownSubcommand { .. }
    ...
    fn test_record_missing_subcommand() {
        let result = parse_command(&test_args("record"), &default_flags());
        assert!(result.is_err());
        assert!(matches!(
            result.unwrap_err(),
            ParseError::MissingArguments { .. }
    ...
    fn test_profiler_start() {
        let cmd = parse_command(&test_args("profiler start"), &default_flags()).unwrap();
    ...
    fn test_profiler_start_with_categories() {
        let cmd = parse_command(
            &test_args("profiler start --categories devtools.timeline,v8.execute"),
            &default_flags(),
        )
        .unwrap();
        assert_eq!(cmd["action"], "profiler_start");
    ...
    fn test_profiler_start_categories_missing_value() {
        let result = parse_command(&test_args("profiler start --categories"), &default_flags());
        assert!(result.is_err());
        assert!(matches!(
            result.unwrap_err(),
            ParseError::MissingArguments { .. }
    ...
    fn test_profiler_stop_with_path() {
        let cmd = parse_command(&test_args("profiler stop trace.json"), &default_flags()).unwrap();
    ...
    fn test_profiler_stop_no_path() {
        let cmd = parse_command(&test_args("profiler stop"), &default_flags()).unwrap();
        assert_eq!(cmd["action"], "profiler_stop");
    ...
    fn test_profiler_invalid_subcommand() {
        let result = parse_command(&test_args("profiler foo"), &default_flags());
        assert!(result.is_err());
        assert!(matches!(
            result.unwrap_err(),
            ParseError::UnknownSubcommand { .. }
    ...
    fn test_profiler_missing_subcommand() {
        let result = parse_command(&test_args("profiler"), &default_flags());
        assert!(result.is_err());
        assert!(matches!(
            result.unwrap_err(),
            ParseError::MissingArguments { .. }
    ...
    #[test]
    fn test_eval_basic() {
        let cmd = parse_command(&test_args("eval document.title"), &default_flags()).unwrap();
    ...
    fn test_eval_base64_invalid() {
        let result = parse_command(&test_args("eval -b !!!invalid!!!"), &default_flags());
        assert!(result.is_err());
        let err = result.unwrap_err();
    ...
    fn test_unknown_command() {
        let result = parse_command(&test_args("unknowncommand"), &default_flags());
        assert!(result.is_err());
        assert!(matches!(
            result.unwrap_err(),
    ...
    fn test_get_missing_subcommand() {
        let result = parse_command(&test_args("get"), &default_flags());
        assert!(result.is_err());
        let err = result.unwrap_err();
    ...
    fn test_get_unknown_subcommand() {
        let result = parse_command(&test_args("get foo"), &default_flags());
        assert!(result.is_err());
        let err = result.unwrap_err();
    ...
    fn test_get_text_missing_selector() {
        let result = parse_command(&test_args("get text"), &default_flags());
        assert!(result.is_err());
        let err = result.unwrap_err();
        assert!(matches!(err, ParseError::MissingArguments { .. }));
    ...
    #[test]
    fn test_mouse_wheel() {
        let cmd = parse_command(&test_args("mouse wheel 100 50"), &default_flags()).unwrap();
    ...
    #[test]
    fn test_set_media() {
        let cmd = parse_command(&test_args("set media dark"), &default_flags()).unwrap();
    ...
    fn test_find_first_no_value() {
        let cmd = parse_command(&test_args("find first a click"), &default_flags()).unwrap();
    ...
    fn test_find_nth_no_value() {
        let cmd = parse_command(&test_args("find nth 2 a click"), &default_flags()).unwrap();
        assert_eq!(cmd["action"], "nth");
        assert_eq!(cmd["index"], 2);
        assert!(cmd.get("value").is_none());
    }
    ...
    #[test]
    fn test_download() {
        let cmd = parse_command(&test_args("download #btn ./file.pdf"), &default_flags()).unwrap();
    ...
    fn test_download_missing_path() {
        let result = parse_command(&test_args("download #btn"), &default_flags());
        assert!(result.is_err());
        assert!(matches!(
            result.unwrap_err(),
            ParseError::MissingArguments { .. }
    ...
    fn test_download_missing_selector() {
        let result = parse_command(&test_args("download"), &default_flags());
        assert!(result.is_err());
        assert!(matches!(
            result.unwrap_err(),
            ParseError::MissingArguments { .. }
    ...
    fn test_wait_download() {
        let cmd = parse_command(&test_args("wait --download"), &default_flags()).unwrap();
    ...
    fn test_wait_download_with_path_and_timeout() {
        let cmd = parse_command(
            &test_args("wait --download ./file.pdf --timeout 30000"),
            &default_flags(),
        )
        .unwrap();
        assert_eq!(cmd["action"], "waitfordownload");
    ...
    fn test_wait_download_short_flag() {
        let cmd = parse_command(&test_args("wait -d ./file.pdf"), &default_flags()).unwrap();
        assert_eq!(cmd["action"], "waitfordownload");
        assert_eq!(cmd["path"], "./file.pdf");
    }
    ...
    fn test_connect_with_port() {
        let cmd = parse_command(&test_args("connect 9222"), &default_flags()).unwrap();
    ...
    fn test_connect_missing_argument() {
        let result = parse_command(&test_args("connect"), &default_flags());
        assert!(result.is_err());
        assert!(matches!(
            result.unwrap_err(),
            ParseError::MissingArguments { .. }
    ...
    fn test_connect_invalid_port() {
        let result = parse_command(&test_args("connect notanumber"), &default_flags());
        assert!(result.is_err());
        let err = result.unwrap_err();
        assert!(matches!(err, ParseError::InvalidValue { .. }));
    ...
    fn test_connect_port_zero() {
        let result = parse_command(&test_args("connect 0"), &default_flags());
        assert!(result.is_err());
        let err = result.unwrap_err();
        assert!(matches!(err, ParseError::InvalidValue { .. }));
    ...
    fn test_connect_port_out_of_range() {
        let result = parse_command(&test_args("connect 65536"), &default_flags());
        assert!(result.is_err());
        let err = result.unwrap_err();
        assert!(matches!(err, ParseError::InvalidValue { .. }));
    ...
    fn test_connect_port_max_valid() {
        let cmd = parse_command(&test_args("connect 65535"), &default_flags()).unwrap();
        assert_eq!(cmd["action"], "launch");
    ...
    fn test_connect_port_min_valid() {
        let cmd = parse_command(&test_args("connect 1"), &default_flags()).unwrap();
        assert_eq!(cmd["action"], "launch");
    ...
    #[test]
    fn test_trace_start() {
        let cmd = parse_command(&test_args("trace start"), &default_flags()).unwrap();
        assert_eq!(cmd["action"], "trace_start");
    }

    #[test]
    fn test_trace_stop_with_path() {
        let cmd = parse_command(&test_args("trace stop ./trace.zip"), &default_flags()).unwrap();
    ...
    fn test_trace_stop_without_path() {
        let cmd = parse_command(&test_args("trace stop"), &default_flags()).unwrap();
        assert_eq!(cmd["action"], "trace_stop");
    ...
    fn test_diff_snapshot_basic() {
        let cmd = parse_command(&test_args("diff snapshot"), &default_flags()).unwrap();
    ...
    fn test_diff_snapshot_baseline() {
        let cmd = parse_command(
            &test_args("diff snapshot --baseline before.txt"),
            &default_flags(),
        )
        .unwrap();
        assert_eq!(cmd["action"], "diff_snapshot");
    ...
    fn test_diff_snapshot_selector_compact_depth() {
        let cmd = parse_command(
            &test_args("diff snapshot --selector #main --compact --depth 3"),
            &default_flags(),
        )
        .unwrap();
        assert_eq!(cmd["action"], "diff_snapshot");
    ...
    fn test_diff_snapshot_short_flags() {
        let cmd = parse_command(
            &test_args("diff snapshot -b snap.txt -s .content -c -d 2"),
            &default_flags(),
        )
        .unwrap();
        assert_eq!(cmd["action"], "diff_snapshot");
    ...
    fn test_diff_screenshot_baseline() {
        let cmd = parse_command(
            &test_args("diff screenshot --baseline before.png"),
            &default_flags(),
        )
        .unwrap();
    ...
    fn test_diff_screenshot_all_options() {
        let cmd = parse_command(
            &test_args("diff screenshot --baseline b.png --output d.png --threshold 0.2 --selector #hero --full"),
            &default_flags(),
        )
        .unwrap();
        assert_eq!(cmd["action"], "diff_screenshot");
    ...
    fn test_diff_screenshot_missing_baseline() {
        let result = parse_command(&test_args("diff screenshot"), &default_flags());
        assert!(result.is_err());
        assert!(matches!(
            result.unwrap_err(),
            ParseError::MissingArguments { .. }
    ...
    fn test_diff_screenshot_global_full_flag() {
        let mut flags = default_flags();
        flags.full = true;
        let cmd = parse_command(&test_args("diff screenshot --baseline b.png"), &flags).unwrap();
        assert_eq!(cmd["action"], "diff_screenshot");
        assert_eq!(cmd["fullPage"], true);
    }
    ...
    fn test_diff_missing_subcommand() {
        let result = parse_command(&test_args("diff"), &default_flags());
        assert!(result.is_err());
        assert!(matches!(
            result.unwrap_err(),
            ParseError::MissingArguments { .. }
    ...
    fn test_diff_unknown_subcommand() {
        let result = parse_command(&test_args("diff invalid"), &default_flags());
        assert!(result.is_err());
        assert!(matches!(
            result.unwrap_err(),
            ParseError::UnknownSubcommand { .. }
    ...
    fn test_diff_snapshot_baseline_missing_value() {
        let result = parse_command(&test_args("diff snapshot --baseline"), &default_flags());
        assert!(result.is_err());
        assert!(matches!(
            result.unwrap_err(),
            ParseError::MissingArguments { .. }
    ...
    fn test_diff_snapshot_selector_missing_value() {
        let result = parse_command(&test_args("diff snapshot --selector"), &default_flags());
        assert!(result.is_err());
        assert!(matches!(
            result.unwrap_err(),
            ParseError::MissingArguments { .. }
    ...
    fn test_diff_snapshot_depth_missing_value() {
        let result = parse_command(&test_args("diff snapshot --depth"), &default_flags());
        assert!(result.is_err());
        assert!(matches!(
            result.unwrap_err(),
            ParseError::MissingArguments { .. }
    ...
    fn test_diff_screenshot_threshold_missing_value() {
        let result = parse_command(
            &test_args("diff screenshot --baseline b.png --threshold"),
            &default_flags(),
        );
        assert!(result.is_err());
        assert!(matches!(
    ...
    fn test_diff_screenshot_output_missing_value() {
        let result = parse_command(
            &test_args("diff screenshot --baseline b.png --output"),
            &default_flags(),
        );
        assert!(result.is_err());
        assert!(matches!(
    ...
    fn test_diff_snapshot_unexpected_arg() {
        let result = parse_command(&test_args("diff snapshot foo"), &default_flags());
        assert!(result.is_err());
        assert!(matches!(
            result.unwrap_err(),
            ParseError::InvalidValue { .. }
    ...
    fn test_diff_screenshot_unexpected_arg() {
        let result = parse_command(
            &test_args("diff screenshot --baseline b.png unexpected"),
            &default_flags(),
        );
        assert!(result.is_err());
        assert!(matches!(
    ...
    fn test_diff_snapshot_unknown_flag() {
        let result = parse_command(&test_args("diff snapshot --invalid"), &default_flags());
        assert!(result.is_err());
        assert!(matches!(
            result.unwrap_err(),
            ParseError::InvalidValue { .. }
    ...
    fn test_diff_url_missing_urls() {
        let result = parse_command(&test_args("diff url"), &default_flags());
        assert!(result.is_err());
        assert!(matches!(
            result.unwrap_err(),
            ParseError::MissingArguments { .. }
    ...
    fn test_diff_snapshot_depth_invalid_value() {
        let result = parse_command(&test_args("diff snapshot --depth abc"), &default_flags());
        assert!(result.is_err());
        assert!(matches!(
            result.unwrap_err(),
            ParseError::InvalidValue { .. }
    ...
    fn test_diff_screenshot_threshold_invalid_value() {
        let result = parse_command(
            &test_args("diff screenshot --baseline b.png --threshold abc"),
            &default_flags(),
        );
        assert!(result.is_err());
        assert!(matches!(
    ...
    fn test_diff_screenshot_threshold_out_of_range() {
        let result = parse_command(
            &test_args("diff screenshot --baseline b.png --threshold 1.5"),
            &default_flags(),
        );
        assert!(result.is_err());
        assert!(matches!(
    ...
    fn test_diff_screenshot_threshold_negative() {
        let result = parse_command(
            &test_args("diff screenshot --baseline b.png --threshold -0.5"),
            &default_flags(),
        );
        assert!(result.is_err());
    }
    ...
    fn test_diff_snapshot_depth_negative_value() {
        let result = parse_command(&test_args("diff snapshot --depth -1"), &default_flags());
        assert!(result.is_err());
        assert!(matches!(
            result.unwrap_err(),
            ParseError::InvalidValue { .. }
    ...
    fn test_scroll_defaults() {
        let cmd = parse_command(&test_args("scroll"), &default_flags()).unwrap();
    ...
    fn test_scroll_direction_and_amount() {
        let cmd = parse_command(&test_args("scroll up 200"), &default_flags()).unwrap();
        assert_eq!(cmd["action"], "scroll");
    ...
    fn test_scroll_with_selector() {
        let cmd = parse_command(
            &test_args("scroll down 500 --selector div.scroll-container"),
            &default_flags(),
        )
        .unwrap();
        assert_eq!(cmd["action"], "scroll");
    ...
    fn test_scroll_selector_missing_value() {
        let result = parse_command(&test_args("scroll down 500 --selector"), &default_flags());
        assert!(result.is_err());
        assert!(matches!(
            result.unwrap_err(),
            ParseError::MissingArguments { .. }
    ...
    fn test_nst_browser_list() {
        let cmd = parse_command(&test_args("nst browser list"), &default_flags()).unwrap();
