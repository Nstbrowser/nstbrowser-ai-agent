...
export class IOSManager {
  ...
  async launch(
    ...
    if (options.udid) {
      ...
      if (!device) {
        throw new Error(
          `Device "${options.device}" not found. Run: nstbrowser-ai-agent device list`
        );
      }
    } else {
      ...
      if (envUdid) {
        ...
        if (!device) {
          throw new Error(
            `Device with UDID ${envUdid} not found. Run: nstbrowser-ai-agent device list`
          );
        }
      } else if (envDevice) {
        device = await this.findDevice(envDevice);
        if (!device) {
          throw new Error(`Device "${envDevice}" not found. Run: nstbrowser-ai-agent device list`);
        }
      } else {
