...
import {
  ...
  isNstbrowserRunning,
  startNstbrowserClient,
  getNstbrowserInstallInstructions,
} from './nstbrowser-utils.js';
...
export class BrowserManager {
  ...
  private async connectToNstbrowser(options?: LaunchCommand): Promise<void> {
    ...
    if (!isRunning) {
      ...
      if (!started) {
        throw new Error(
          'Failed to start Nstbrowser client automatically. ' +
  ...
  async launch(options: LaunchCommand): Promise<void> {
    ...
    if (hasExtensions) {
      ...
      context = await launcher.launchPersistentContext(
        path.join(os.tmpdir(), `nstbrowser-ai-agent-ext-${session}`),
        {
          headless: false,
  ...
  async startRecording(outputPath: string, url?: string): Promise<void> {
    ...
    this.recordingTempDir = path.join(
      os.tmpdir(),
      `nstbrowser-ai-agent-recording-${session}-${Date.now()}`
    );
