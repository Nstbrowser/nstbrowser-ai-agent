...
describe('getSocketDir', () => {
  ...
  describe('NSTBROWSER_AI_AGENT_SOCKET_DIR', () => {
    it('should use custom path when set', () => {
      process.env.NSTBROWSER_AI_AGENT_SOCKET_DIR = '/custom/socket/path';
      expect(getSocketDir()).toBe('/custom/socket/path');
    });

    it('should ignore empty string', () => {
      process.env.NSTBROWSER_AI_AGENT_SOCKET_DIR = '';
      const result = getSocketDir();
      expect(result).toContain('.nstbrowser-ai-agent');
    });

    it('should take priority over XDG_RUNTIME_DIR', () => {
      process.env.NSTBROWSER_AI_AGENT_SOCKET_DIR = '/custom/path';
  ...
  describe('XDG_RUNTIME_DIR', () => {
    it('should use when NSTBROWSER_AI_AGENT_SOCKET_DIR is not set', () => {
      process.env.XDG_RUNTIME_DIR = '/run/user/1000';
      expect(getSocketDir()).toBe('/run/user/1000/nstbrowser-ai-agent');
    });
  ...
  describe('fallback', () => {
    it('should use home directory when env vars are not set', () => {
      const result = getSocketDir();
      const expected = path.join(os.homedir(), '.nstbrowser-ai-agent');
