...
describe('auth-vault', () => {
  beforeEach(() => {
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), 'nstbrowser-ai-agent-auth-test-'));
    delete process.env.NSTBROWSER_AI_AGENT_ENCRYPTION_KEY;
  });
  ...
  function cleanAuthDir() {
    const authDir = path.join(tempHome, '.nstbrowser-ai-agent', 'auth');
  ...
  describe('auto-generated encryption key', () => {
    it('should auto-create key file and encrypt profile when no env var is set', () => {
      ...
      const keyFilePath = path.join(tempHome, '.nstbrowser-ai-agent', '.encryption-key');
      ...
      const profilePath = path.join(tempHome, '.nstbrowser-ai-agent', 'auth', 'autokey.json');
