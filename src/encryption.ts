...
export function getKeyFilePath(): string {
  return join(os.homedir(), '.nstbrowser-ai-agent', KEY_FILE_NAME);
}
 ...
 *
 * Checks (in order):
 * 1. NSTBROWSER_AI_AGENT_ENCRYPTION_KEY env var
 * 2. ~/.nstbrowser-ai-agent/.encryption-key file
 *
 ...
 * and writes it to ~/.nstbrowser-ai-agent/.encryption-key (mode 0600).
 */
export function ensureEncryptionKey(): Buffer {
  ...
  const dir = join(os.homedir(), '.nstbrowser-ai-agent');
  if (!existsSync(dir)) {
  ...
  console.error(
    `[nstbrowser-ai-agent] Auto-generated encryption key at ${keyFilePath} -- back up this file or set ${ENCRYPTION_KEY_ENV}`
  );
